    ...
    for (0..content.len) |i| {
        ...
        if (rangeFound) {
            ...
            for (rangeStart..rangeEnd + 1) |num| {
                ...
                var currentDivisor: usize = 2;
                var testSize = middleIndex;
                if (!isEvenLength) {
                    testSize = endIndex / 3;
                    currentDivisor = 3;
                }
                ...
                while (testSize >= 1) {
                    var l: usize = 0;
                    var r: usize = testSize;
                    var found = false;
                    while (r + testSize <= stringNum.len) {
                        if (!std.mem.eql(u8, stringNum[l..r], stringNum[r .. r + testSize])) {
                            break;
                        }

                        if (r + testSize == endIndex) {
                            sumOfInvalid += num;
                            found = true;
                        }
                        r += testSize;
                    ...
                    if (found) break;
                    currentDivisor += 1;
                    testSize = endIndex / currentDivisor;
                }
            }
